	...
	local t =
		...
		JudgmentMessageCommand = function(self, params)
			...
			if params.Player == pn then
				if
					params.TapNoteScore and params.TapNoteScore ~= "TapNoteScore_HitMine" or
						params.TapNoteScore ~= "TapNoteScore_AvoidMine"
				 then
	...
	if enabled.NPSDisplay[pn] then
		t[#t + 1] =
			LoadFont("Common Normal") ..
			{
				...
				InitCommand = function(self)
					self:x(npsDisplayX):y(npsDisplayY):halign(0):zoom(npsDisplayZoom):halign(0):valign(0):shadowlength(1):settext(
						"0.0 NPS"
					)
				end,
