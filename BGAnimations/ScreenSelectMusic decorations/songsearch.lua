...
t = Def.ActorFrame{
	...
	Def.Quad {
		InitCommand=cmd(xy,xpos+200,ypos;zoomto,SCREEN_WIDTH*8/20,200;visible,false;);
		SetCommand=function(self)
	...
	Def.Quad {
		InitCommand=cmd(xy,xpos+200,ypos);
		OnCommand=cmd(diffuse,color("#AA7711");zoomto,SCREEN_WIDTH/3,45;visible, false;diffusealpha,0.75);
		SetCommand=function(self) 
			if active then
				self:visible(true)
			else
	...
	LoadFont("Common Large")..{
		InitCommand=cmd(xy,xpos+SCREEN_WIDTH/6,ypos-60;zoom,0.7;halign,0.5;maxwidth,SCREEN_WIDTH/3;shadowlength,1;),
		SetCommand=function(self) 
			if active then
				self:settext("Search")
			else
	...
	LoadFont("Common Large")..{
		InitCommand=cmd(xy,xpos+200,ypos;zoom,0.6;halign,0.5;maxwidth,550;shadowlength,1;),
		SetCommand=function(self) 
			if active then
