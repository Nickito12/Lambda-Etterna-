...
local base_options = {
	...
	{
		name = "reload_noteskins",
		translatable = true,
		type = "action",
		execute = function()
			NOTESKIN:reload_skins()
		end
	}
}
...
local function input(event)
	...
	if not menus[pn] then
		return
	end
	if menu_stack_generic_input(menus, event) and event.type == "InputEventType_FirstPress" then
...
for pn, menu in pairs(menus) do
	frame[#frame + 1] =
		LoadActor(THEME:GetPathG("ScreenOptions", "halfpage")) ..
		{
	...
	frame[#frame + 1] =
		...
		InitCommand = function(self)
			...
			self:xy(menu_x[pn] - (menu_width / 2), _screen.cy + 154):diffuse(ColorDarkTone((PlayerColor(pn)))):horizalign(left):vertalign(
				top
			):wrapwidthpixels(menu_width / .8):zoom(.75):horizalign(left)
		end,
	...
	frame[#frame + 1] =
		LoadActor(THEME:GetPathG("ScreenPlayerOptions", "PlayerNameplate"), pn) ..
		{
			InitCommand = function(self)
