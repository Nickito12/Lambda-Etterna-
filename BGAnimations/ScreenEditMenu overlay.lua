...
local stype_item_mt =
	edit_pick_menu_steps_display_item(
	stype_transform,
	Def.BitmapText {
	...
	Def.BitmapText {
		...
		SetCommand = function(self, param)
			self:settext(param.steps:GetMeter()):diffuse(ColorDarkTone(GameColor.Difficulty[param.steps:GetDifficulty()]))
...
local frame =
	Def.ActorFrame {
	EditMenuSelectionChangedMessageCommand = edit_pick_menu_update_steps_display_info(steps_display),
